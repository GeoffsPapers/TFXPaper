...
third arguments. The type of the conditional expression is the type of its second and third arguments. The following is the relevant BNF excerpt.
\begin{verbatim}
...
Each let expression in TFX consists of (i) the type signatures of the defined symbols; (ii) the definitions of the symbols; and (iii) the formula or term in which the definitions are applied. Type signatures in let expressions syntactically match those in annotated formulas with the \verb'type' role. Definitions of symbols include one or more function symbol, predicate symbol, or tuple, defined simultaneously. The type signature must include all function symbols, predicate symbols, and elements of tuples, used in the definitions. The following is the relevant BNF excerpt.
\begin{verbatim}
...
Let expression can use definitions of tuples. Formula~\ref{eq:parallel-let-tuple-example} can be written in TFX syntax as following.
\begin{verbatim}
tff(a,type,a: $i).
tff(b,type,b: $i).
tff(p,type,p: ($i * $i) > $o).
...
Formula~\ref{eq:fool-tuple-let} can be written in TFX as follows. Notice that the type declaration contains the elements of both tuples in the simultaneous definition.
\begin{verbatim}
