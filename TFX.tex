...
that defines first class tuple terms and first class tuple types. 
...
variation of many-sorted first-order logic. 
FOOL extends ordinary FOL in that it (i)~contains an interpreted boolean sort 
and allows formulae to be used as boolean terms, (ii)~allows boolean variables 
to be used as formulae, (iii)~contains conditional expressions, and 
(iv)~contains let expressions with definitions of function symbols, predicate 
symbols, and tuples. 
FOOL can be straightforwardly extended with the polymorphic theory of tuples 
that defines first class tuples type and first class tuple terms \cite{KKV18}.
In what follows we consider that extension, and first class tuple terms and 
tuple types are part of TFX.
...
possible to reason in FOOL using a regular first-order theorem prover. 
...
\subsubsection{Boolean variables as formulae}
...
\subsubsection{Formulae as boolean terms}
...
$A$ and $B$ using a local definition of the predicate symbol $\mathit{imply}$:
...
is equivalent to $P(b, a)$.
\begin{equation}\label{eq:parallel-let-example}
\mathtt{let}\;\binding{a}{b}\,;\,\binding{b}{a}\;\mathtt{in}\;P(a, b)
\end{equation}

Formula~\ref{eq:parallel-let-example} can be equivalently expressed using a let expression with a definition of a tuple as follows.
...
The type of the $\mathit{imply}$ predicate in Formula~\ref{eq:fool-imply} can 
...
Each let expression in TFX consists of (i) the type signatures of the defined 
symbols; (ii) the definitions of the symbols; and (iii) the formula or term 
...
Thus Formula~\ref{eq:fool-let-imply} can be written in TFX syntax as:
\begin{verbatim}
...
% Function symbols, predicate symbols, and tuples can all be defined 
% simultaneously. 
...
and systems that can already parse and reason using the SMT version 2 syntax 
need only new parsers (for TFX syntax) to implement the features.
